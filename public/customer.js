...
document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch("https://findhelper.onrender.com/professionals")
...
function loadCustomerProfile() {
  fetch(`https://findhelper.onrender.com/customers?email=${user.email}`)
    .then((res) => res.json())
    .then((customers) => {
...
function openBookingsModal() {
  fetch(`https://findhelper.onrender.com/bookings?customerEmail=${user.email}`)
    .then((res) => res.json())
    .then((bookings) => {
      ...
      const professionalPromises = bookings.map((booking) => {
        return fetch(
          `https://findhelper.onrender.com/professionals/${booking.professionalId}`
        )
          .then((res) => res.json())
          .then((professional) => {
            return {
...
function updateProfile() {
  ...
  fetch(`https://findhelper.onrender.com/customers?email=${user.email}`)
    ...
    .then((customers) => {
      if (customers.length > 0) {
        const customer = customers[0];
        return fetch(`http://localhost:10000/customers/${customer.id}`, {
          method: "PATCH",
    ...
    .then(() => {
      // Update user data
      return fetch(`https://findhelper.onrender.com/users/${user.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
...
function loadCompletedBookings() {
  ...
  fetch(
    `https://findhelper.onrender.com/bookings?customerEmail=${encodeURIComponent(
      user.email
    )}&status=completed`
  )
    .then((res) => res.json())
...
function submitReview(bookingId) {
  ...
  fetch(`https://findhelper.onrender.com/bookings/${bookingId}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
